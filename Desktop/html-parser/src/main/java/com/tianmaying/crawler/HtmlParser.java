...
public class HtmlParser {
    ...
    public List<WebPage> parsePlaylists(String url) {
        /*String body = HTML_FETCHER.fetch(url);
    ...
    public List<WebPage> parsePlaylist(String url) {
        // your code here
        Document document = Jsoup.parse(HTML_FETCHER.fetch(url));
        Elements playlist = document.select("ul.f-hide > li > a");
        //System.out.println(playlist.html());
        return playlist.stream().map(e -> new WebPage(BASE_URL + e.attr("href"), PageType.song)).collect(Collectors.toList());
        //return null;
    }
